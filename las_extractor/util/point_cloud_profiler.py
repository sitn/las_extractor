...
# Class names corresponding to SITN LiDAR codes
def classificationNames(dataType):

    classesNamesStandard = {
    ...
    if dataType == 'standard':
        return classesNamesStandard
    elif dataType == 'normalized':
        return classesNamesNormalized
...
def generate_json(profile, jsonOutput, csvOut, classesList, classesNames):
    ...
    for row in profile:
        id += 1
        classifName = classesNames[str(row[4])]
        # csv output
        ...
        # json output
        jsonOutput.append({'dist':math.ceil(row[0] * 1000) / 1000,\
            'values':{serie:math.ceil(row[1] * 1000) / 1000},'x':row[2],'y':row[3]})
...
def csv2kml(csvFile,markerUrl,outputKml,classesNames):
        """
        ...
        for row in reader:  
            ...
            kmlPoint.style.iconstyle.icon.href = markerUrl + kmlColors[str(row[4])]
        kml.save(outputKml)
        return True
